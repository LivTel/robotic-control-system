...
public class CalibrationControlAgent extends DefaultModalTask
{
	...
	public void configure(File file) throws IOException, IllegalArgumentException
	{
		...
		try
		{
			...
			if (config.getProperty("focus.star.catalog") != null)
			{
				...
				taskLog.log(1, CLASS, name, "configure", "CalibrationControlAgent loaded "+
					    focusStarCatalog.size()+" targets from "
			...
			/** The sun level at which we stop morning flats. */
			// Check instrument blank catalogs
			...
			while (it.hasNext())
			{
				...
				if (config.getProperty(ilcname + ".blank.catalog") != null)
				{
					...
					taskLog.log(1, CLASS, name, "configure", "CalibrationControlAgent loaded "+
						    instBlankCatalog.size()+" targets from "+ instBlankCatalog.getCatalogName());
			...
			taskLog.log(1, CLASS, name, "configure", "CalibrationControlAgent EveningSkyFlats Min duration: " + (eveningSkyFlatsMinimumDuration / 1000L) + "s");
			taskLog.log(1, CLASS, name, "configure",
					"CalibrationControlAgent EveningSkyFlats Max Solar: Start at: " + Math.toDegrees(eveningSkyflatsMaximumSolarElevation) + "deg");
			taskLog.log(1, CLASS, name, "configure",
					"CalibrationControlAgent EveningSkyFlats Min Solar: Stops at: " + Math.toDegrees(eveningSkyflatsMinimumSolarElevation) + "deg");

			taskLog.log(1, CLASS, name, "configure", "CalibrationControlAgent MorningSkyFlats Min duration: " + (morningSkyFlatsMinimumDuration / 1000L) + "s");
			taskLog.log(1, CLASS, name, "configure",
					"CalibrationControlAgent MorningSkyFlats Min Solar: Start at: " + Math.toDegrees(morningSkyflatsMinimumSolarElevation) + "deg");
			taskLog.log(1, CLASS, name, "configure",
					"CalibrationControlAgent MorningSkyFlats Max Solar: Stops at: " + Math.toDegrees(morningSkyflatsMaximumSolarElevation) + "deg");
			...
			taskLog.log(1,CLASS,name,"configure","CalibrationControlAgent Calib weights: same="+calibWeightSameFlats+", other="
		...
		taskLog.log(1,CLASS,name,"configure","CalibrationControlAgent was configured ok");
	}
	...
	public long nextWantsControl(long time) throws RemoteException
	{
		...
		taskLog.log(1,CLASS,name,"nextWantsControl",
			    "CalibrationControlAgent::nextWantsControl::Checking ahead to see if wanting control...");
		long t = time;
	...
	public boolean wantsControl(long time) throws RemoteException
	{
		...
		try {
			...
			{
				date = new Date(time);
				taskLog.log(1,CLASS,name,"wantsControl",
					    "CalibrationControlAgent::wantsControl::At "+date+" Found these jobs: "+list.toString());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * 
	 * // MorningSkyFlats if (morning && ical.doMorningSkyflatCalibration() && (time -
	 ...
	 * if ((!istat.isOnline()) && (!istat.isFunctional())) continue;
	 * 
	 * // EveningSkyFlats if (evening && ical.doEveningSkyflatCalibration() && (time -
	...
	/**
	 * Find a list of calibration jobs that can be executed at the specified time.
	 * @param time The time we want to do a calibration task at, a long in milliseconds since the epoch.
	...
	private List<CalibrationOperation> findAvailableJobs(long time) throws Exception
	{
		...
		double score, sameScore, otherScore, randomScore, priorityScore;
		...
		if (logsearch)
		{
			taskLog.log(1,CLASS,name,"findAvailableJobs",
				    "CalibrationControlAgent::findAvailableJobs:Sunlev is now: "+
				    Math.toDegrees(sunlev)+" ->  "+Math.toDegrees(sunlevplus));
		}
		...
		if (morningTwilight)
		{
			...
			if (sunlev > morningSkyflatsMinimumSolarElevation && sunlev < morningSkyflatsMaximumSolarElevation)
			{
				...
				{
					taskLog.log(1,CLASS,name,"findAvailableJobs",
						    "CalibrationControlAgent::findAvailableJobs:Test for MorningSkyFlats...");
				}
				...
				while (ii.hasNext())
				{
					...
					{
						taskLog.log(1,CLASS,name,"findAvailableJobs",
							    "CalibrationControlAgent::findAvailableJobs:check instrument: " + isp);
					}
					// skip if the instrument is shagged !
					if (!inst.isFunctional() || !inst.isOnline() || !inst.isEnabled())
					{
						taskLog.log(1,CLASS,name,"findAvailableJobs",
							    "CalibrationControlAgent::findAvailableJobs:instrument: "+
							    id.getInstrumentName()+
							    " is broken: isFunctional:"+inst.isFunctional()+
					...
					taskLog.log(1,CLASS,name,"findAvailableJobs",
						    "CalibrationControlAgent::findAvailableJobs:check calib: " +
						    id.getInstrumentName() + " " + ical + " history: " + ich);

					if (ical == null)
					{
						taskLog.log(1,CLASS,name,"findAvailableJobs",
							    "CalibrationControlAgent::findAvailableJobs:instrument: "+
							    id.getInstrumentName() + " has no instrument calibration requirements.");
						continue;
					}
					...
					// check if MorningSkyFlats is feasible...
					...
					taskLog.log(1,CLASS,name,"findAvailableJobs",
						    "CalibrationControlAgent::findAvailableJobs:instrument: " +
						    id.getInstrumentName()+
						    " Morning Sky Flats feasable "+mfeas+
						    " with doMsf: "+doMsf+
						    " timing:"+(timeTillSunMax > morningSkyFlatsMinimumDuration)+
						    ","+(sunlev > morningSkyflatsMinimumSolarElevation)+","+(msfLast > msfInt));

					// if (logsearch)
					// taskLog.log(1,CLASS,name,"findAvailableJobs","findAvailableJobs::tssat=" +
					// timeSinceStartAstroTwilight + ", msf-ST-Off=" + msfSt
					// + ", msf-ET-Off=" + msfEt + ", feasible: " + mfeas);

					// taskLog.log(1,CLASS,name,"findAvailableJobs","Find available: (MorningSkyFlats): Sunlev: %4.2f,
					// Min: %4.2f, Max: %4.2f
					
					// calculate same score, check getMorningSkyflatCalibrationInterval to stop division by zero
					if(ical.getMorningSkyflatCalibrationInterval() > 0)
					{
						sameScore = calibWeightSameFlats
					...
					// calculate other score, check getEveningSkyflatCalibrationInterval to stop division by zero
					if(ical.getEveningSkyflatCalibrationInterval() > 0)
					{
						otherScore = calibWeightOtherFlats
					...
					randomScore = (calibWeightRandom * Math.random());
					priorityScore = (calibWeightPriority * ical.getCalibrationPriority());
					score = sameScore + otherScore + randomScore + priorityScore;
					taskLog.log(1,CLASS,name,"findAvailableJobs",
						    "CalibrationControlAgent::findAvailableJobs:instrument: " +
						    id.getInstrumentName()+
						    " Morning Sky Flats score "+score+
						    " sameScore: "+sameScore+
					...
					if (mfeas)
					{
						taskLog.log(1,CLASS,name,"findAvailableJobs",
							    "CalibrationControlAgent::findAvailableJobs:instrument: " +
							    id+" Morning Sky Flats are feasible:Adding to list with score "+score);
						list.add(new MorningSkyFlatCalibration(id, score));
					}
				}// end while instrument descriptors
			}// end if sunelv is in range
		}
		else if (eveningTwilight)
		{
			...
			if (sunlev > eveningSkyflatsMinimumSolarElevation && sunlev < eveningSkyflatsMaximumSolarElevation)
			{
				...
				{
					taskLog.log(1,CLASS,name,"findAvailableJobs",
						    "CalibrationControlAgent::findAvailableJobs:Test for EveningSkyFlats...");
				}
				// how long since SAT and until TSR
				...
				while (ii.hasNext())
				{
					...
					// check if EveningSkyFlats is feasible...
					...
					taskLog.log(1,CLASS,name,"findAvailableJobs",
						    "CalibrationControlAgent::findAvailableJobs:instrument: " +
						    id.getInstrumentName()+
						    " Evening Sky Flats feasable "+efeas+
						    " with doEsf: "+doEsf+
						    " timing:"+(timeTillSunMin > eveningSkyFlatsMinimumDuration)+
						    ","+(sunlev < eveningSkyflatsMaximumSolarElevation)+","+(esfLast > esfInt));

					// if (logsearch)
					// System.err.println("findAvailableJobs::tssset=" + timeSinceSunset +
					// ", esf-ST-Off=" + esfSt + ", esf-ET-Off="
					// + esfEt + ", feasible: " + efeas);
					
					// calculate same score, check getEveningSkyflatCalibrationInterval to stop division by zero
					if(ical.getEveningSkyflatCalibrationInterval() > 0)
					{
						sameScore = calibWeightSameFlats *
							Math.floor((double)(time-ich.getLastEveningSkyflatCalibration())
					...
					// calculate other score, check getMorningSkyflatCalibrationInterval to stop division by zero
					if(ical.getMorningSkyflatCalibrationInterval() > 0)
					{
						otherScore = calibWeightOtherFlats
							* Math.floor((double) (time - ich.getLastMorningSkyflatCalibration())
					...
					if (efeas)
					{
						taskLog.log(1,CLASS,name,"findAvailableJobs",
							    "CalibrationControlAgent::findAvailableJobs:instrument: " +
							    id+" Evening Sky Flats are feasible:Adding to list with score "+score);
		...
		else
		{
			...
			// TEMP change to 5 am
			boolean round_midnight_and_m_le_10 = (((h == 23) && (m >= 50)) || ((h == 0) && ((m <= 10) || (m >= 50))) ||
							      ((h == 1) && (m < 10)));
			...
			{
				taskLog.log(1,CLASS,name,"findAvailableJobs",
					    "CalibrationControlAgent findAvailableJobs:checking for telfocus: ttSR = " +
					    timeTillSunrise + " tsSS = "+ timeSinceSunset);
			}
		...
		try
		{
			...
			while (il.hasNext())
			{
				...
				taskLog.log(1, CLASS, name, "getNextJob",
					    "CalibrationControlAgent::getNextJob:Checking job: " + job);
				if (job.getScore() > bestscore)
				{
					bestscore = job.getScore();
					bestjob = job;
					taskLog.log(1, CLASS, name, "getNextJob",
						    "CalibrationControlAgent::getNextJob: New best job: " + job + " with score:" + bestscore);
				}
			}
		}
		catch (Exception e)
	...
	private SkyFlatCalibrationTask createMorningSkyFlatsTask(MorningSkyFlatCalibration msf) throws Exception
	{
		...
		taskLog.log(1, "CalibrationControlAgent:: Check calibration history for: " + instId + " " + ich);
		...
		InstrumentStatusProvider isp = ireg.getStatusProvider(instId);
		InstrumentStatus inst = isp.getStatus();
		// skip if the instrument is broken
		if (!inst.isFunctional() || !inst.isOnline() || !inst.isEnabled())
		{
			throw new Exception("CalibrationControlAgent:: Instrument "+instId+
					    " not available for MorningSkyFlats: isFunctional: "+inst.isFunctional()+
					    " isOnline: "+inst.isOnline()+" isEnabled: "+inst.isEnabled());

		}
		long timeAvailable = timeTillSunMax;

		taskLog.log(1,"CalibrationControlAgent:: create MorningSkyFlats task: sunat: " + sunlev + ", ttmax("
	...
	private SkyFlatCalibrationTask createEveningSkyFlatsTask(EveningSkyFlatCalibration esf) throws Exception
	{
		...
		{
			throw new Exception("CalibrationControlAgent:: Instrument "+instId+
					    " not available for EveningSkyFlats: isFunctional: "+inst.isFunctional()+
					    " isOnline: "+inst.isOnline()+" isEnabled: "+inst.isEnabled());
		}
		long timeAvailable = timeTillSunMin;

		taskLog.log(1,"CalibrationControlAgent:: create EveningSkyFlats task: sunat: " + sunlev + ", ttmax("
			    + Math.toDegrees(eveningSkyflatsMinimumSolarElevation) + ")deg " + (timeTillSunMin / 1000)+ "s");
	...
	{
		super.onInit();
		opsLog.log(1, "CalibrationControlAgent:: Starting Calibration-Operations Mode.");
	...
	{
		super.onCompletion();
		opsLog.log(1, "CalibrationControlAgent:: Completed Calibration-Operations Mode.");
	}
	...
	public void onSubTaskDone(Task task)
	{
		...
		if (task instanceof TelFocusCalibrationTask)
		{
			...
			opsLog.log(1, "CalibrationControlAgent:: Completed TELFOCUS calibration, updated calibration history");
		}
