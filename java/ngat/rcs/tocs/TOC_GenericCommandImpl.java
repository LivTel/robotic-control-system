...
public class TOC_GenericCommandImpl implements RequestHandler {
	 ...
	 * Set at the end of a successful INSTR and FOCALPLANE command.
	 * Used by the ACQUIRE command so we can get the science instrument capabilities.
	 */
	protected static String selectedInstId;

	/** 
	 * The current InstrumentConfig. 
	 * Set at the end of a successful INSTR command.
	 * Used by the EXPOSE and ARC commands so they know which instrument to send the EXPOSE/ARC command to.
	 */
	...
	public void processHELPCommand(StringTokenizer parser) 
	{
		reply = "The following commands are permitted:" + "\nHELO <serviceID>   : Lets the TOC_Server identify client."
				...
				+ "\nACQUIRE <sessionID> <ra> <dec> <mode> <precision>"
...
//				+ "\n                     selection and wavelengths plus a window size."
	 ...
	 * ACQUIRE &lt;sessionID> &lt;ra&gt; &lt;dec&gt; &lt;mode&gt; &lt;precision&gt;
	 * &lt;mode&gt; is either WCS or BRIGHTEST
	 * &lt;precision&gt;is either NORMAL or HIGH
	 */
	public void processACQUIRECommand(StringTokenizer parser) 
	{
		...
		{
			processError("MISSING_PARAMETERS", "Use: ACQUIRE <sessionID> <ra> <dec> <mode> <precision>");
			return;
		}
		// Are we in an active session (post HELO)
		if (!checkSession(parser))
			return;
		// parse RA/Dec
		String strRA = parser.nextToken();
		...
		// parse acquisition precision
		String strPrecision = parser.nextToken();
		if (strPrecision.equalsIgnoreCase("NORMAL"))
			highPrecision = false;
		else if (strPrecision.equalsIgnoreCase("HIGH"))
			highPrecision = true;
		else 
		{
			processError("ILLEGAL_ACQUISITION_PRECISION", strPrecision);
			return;
		}
		...
		// get instrument capabilities for currently selected science instrument (last INSTR)
		...
		// Work out a suitable acquisition instrument
		// If the science instrument is Sprat, use Sprat for acquisition
		// Otherwise always use IO:O
		String acqInstId = null;
		if(selectedInstId.equalsIgnoreCase("SPRAT"))
			acqInstId = "SPRAT";
		else
			acqInstId = "IO:O";
        // get acquisition parameters from science instrument acquisition 	
		int ax = 0;
		int ay = 0;		
		...
		try 
		{
			InstrumentDescriptor instrumentDescriptor = new InstrumentDescriptor(acqInstId);
			...
			acquisitionThreshold = icap.getAcquisitionThreshold(instrumentDescriptor, highPrecision);
		} 
		catch (Exception e) 
		{
			e.printStackTrace();
	...
	public void processFOCALPLANECommand(StringTokenizer parser) 
	{
		...
		// Set the selected instrument id to be the instrument name
		// This is used to determine the science instrument in the ACQUIRE command implementation.
		// This means we can ACQUIRE before sending an INSTR command to the science instrument.
		selectedInstId = instumentName;
	}
	...
	public void processINSTRCommand(StringTokenizer parser) {
		...
		// subsequent exposures, what to acquire onto and other stuff...
