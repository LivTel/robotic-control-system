...
public class ISS_OFFSET_FOCUS_CONTROL_CommandImpl extends ISS_CommandImpl {

	public static final String CLASS = "ISS_OFFSET_FOCUS_CONTROL_CommandImpl";
	...
	public ISS_OFFSET_FOCUS_CONTROL_CommandImpl(JMSMA_ProtocolServerImpl serverImpl, COMMAND receivedCommand) {
	...
	public void processReceivedCommand(INST_TO_ISS receivedCommand) {
		...
		if (ISS.getBeamControlInstrument() == null) {
			// no beam controller so anything goes thro
			overrideNoForward = false; // ie we send onwards
		} else if (!instrumentName.equals(ISS.getBeamControlInstrument())) {
			overrideNoForward = true;
			System.err.println("ISS: Instrument does not match beam-control instrument, Not sending focus offset");
			// TODO Not clear if we need to call: setInstrumentFocusOffset in
			// this instance or not ?
			return;
		}

		// check for zero offset - no send onwards
		if (receivedCommand instanceof OFFSET_FOCUS_CONTROL) {
			double cfo = StatusPool.latest().mechanisms.focusOffset;
			double rfo = ((OFFSET_FOCUS_CONTROL) receivedCommand).getFocusOffset();
			double ifo = ISS.getCurrentFocusOffset();
			System.err.println("ISS: Current Focus offset: " + cfo + ", Requested: " + rfo + ", From Obs: " + ifo);
			// basically if the difference between current and requested + obs
			// is > small then send
			if (Math.abs((rfo + ifo) - cfo) < 0.001) {
				overrideNoForward = true;
				System.err.println("ISS: Focus change small, Not sending focus offset");
			} else {
				System.err.println("ISS: Significant focus change, Sending offset");
			}
			// always update instrument requested value
	...
	public COMMAND_DONE makeResponse() {
		OFFSET_FOCUS_CONTROL_DONE done = new OFFSET_FOCUS_CONTROL_DONE("FAKED");
	...
	public COMMAND_DONE translateResponse(COMMAND_DONE response) {
		...
		OFFSET_FOCUS_CONTROL_DONE done = new OFFSET_FOCUS_CONTROL_DONE(receivedCommand.getId());
