...
public class ISS_MOVE_FOLD_RETRY_CommandImpl extends ISS_CommandImpl {
	...
	public ISS_MOVE_FOLD_RETRY_CommandImpl(JMSMA_ProtocolServerImpl serverImpl, COMMAND receivedCommand) {
	...
	public COMMAND_DONE makeResponse() {
		...
		if (rfold != cfold) {
			...
			if (cfold >= 1 && cfold <= 8 && rfold >= 1 && rfold <= 8) {
				...
				sendMove = true;
				System.err.println("ISS_MOVE_FOLD::Outgoing():Fold is currently deployed at port-" + cfold
						+ " , moving to new port-" + rfold);
			} else if (cfold == 0 && rfold >= 1 && rfold <= 8) {
				// stowed but need to deploy and shift
				sendMove = true;
				sendAgf = true;
				agdirn = agFilterDeploy;
				System.err.println("ISS_MOVE_FOLD::Outgoing():Fold is currently stowed, deploying and moving to port-"
						+ rfold);
			} else if (cfold >= 1 && cfold <= 8 && rfold == 0) {
				...
				agdirn = agFilterDeploy;
				System.err
						.println("ISS_MOVE_FOLD::Outgoing():Fold deployment is currently unknown, deploying and moving to port-"
								+ rfold);
			}
			...
			if (sendAgf) {
				...
				// we have several tries, keep track of error messages may need
				// them..
				int tries = 0;
				...
				while (!success && tries < 3) {
					...
					if (tries > 1) {
						...
						if ((agdirn == agFilterStowed && cdeploy == TCS_Status.POSITION_INLINE)
							...
							System.err
									.println("ISS_MOVE_FOLD::Outgoing():Agfilter is now in correct position, cancel attempt");
							success = true;
							continue;
						}
						...
						// send another ack, if it fails were out of here as
						// client is probably gone...
						ACK ack2 = makeAck(150000L); // 60 sec delay, 60 sec
														// command timeout + 30
														// spare
