...
/** As a rule TCS_Status stores its fields in the form (units) they were received from the TCS.
 * E.g. angles (az.pos) etc arre in degrees etc.
 */
public class LT_RGO_SHOW_DONE_Translator implements Translator {
	...
	try {
		...
		//System.err.println("SHOWTRANS::Parsing [SHOW AG] Response: at posn 3: "+parseString   (3)+
		...
		status.primMirrorCoverDemand = TCS_Status.getCode(parseString(21));	
		status.primMirrorCoverPos    = TCS_Status.getCode(parseString(22));	
		status.primMirrorCoverStatus = TCS_Status.getCode(parseString(23));
		...
		status.primMirrorSysStatus   = TCS_Status.getCode(parseString(28));
		...
		status.srcEquinox        = parseEquinox(4); // watch for the start letter. B/J/A		
		...
		status.engineeringOverrideState = TCS_Status.getCode(parseString(2));
		...
		status.timeStamp = System.currentTimeMillis(); //// TEMP should be the UT1 value really !
		xstatus = status;
	    }
	    break;
	    default:
...
// 	}
	...
	if (val.startsWith("J") ||
		...
		errorLog.log(3, "LT_SHOW_DONE", "parseEquinox", 
			     "Error parsing Equinox from key: ["+key+"] = ["+val+"]");
	    }
	    return data;
	} else if
	    (val.startsWith("A"))
