 ...
 * during its init() call. The task needs a specialized callback object to
 * register with the remote server to receive the callback. The
 ...
 * @version $Revision:$
 */
public abstract class AsynchronousRemoteCallTask implements Task {
	
	// ERROR_BASE: RCS = 6, SCIOPS = 60, ASYNCH = 200
	
    /** Stores the classname for this task. */
    protected String CLASS;

    /** Constant indicating a problem while creating the responder. */
    public static final int INTERNAL_ERROR = 660201;

    /** Constant indicating that the remote call failed. */
    public static final int REMOTE_ERROR = 660202;

    /** Constant indicating that the remote call timed-out. */
    public static final int TIMEOUT_ERROR = 660203;
    ...
    /** The current (active) {@link AsynchronousResponseHandler}. */
    protected AsynchronousResponseHandler activeResponder;
    ...
    public AsynchronousRemoteCallTask(String name, TaskManager manager) {
	this.name = name;
    ...
    public void init() {
	try {
	    activeResponder = createActiveResponder();
	} catch (Exception e) {
	    e.printStackTrace();
	    failed = true;
	    errorIndicator.setErrorCode(INTERNAL_ERROR);
	    errorIndicator.setErrorString("Create active responder failed with exception: " + e);
	    if (manager != null)
     ...
     * either timeout or complete. This is neccessary for the Task's Invocation
	...
	// then fail
	try {
	    logger.create().extractCallInfo().info().level(2).msg("Making remote call").send();
	    makeRemoteCall();
	} catch (Exception e) {
	    e.printStackTrace();
	    failed = true;
	...
	if (!isAborted() && !isDone() && !isFailed()) {
	    ...
	    try {
		logger.create().extractCallInfo().info().level(2).msg("Callback monitor Waiting for: " + timeout);
		logger.create().extractCallInfo().info().level(2).msg("Waiting on lock for upto: " + timeout + " ms")
		    .send();
		waitOnLock(timeout);
		...
		logger.create().extractCallInfo().info().level(2).msg("Interrupted while waiting on signal").send();

	    }

	}

	// At this point we have either timed out or been deliberately
	// interrupted-
	...
	// None of the above - weve timed out...
	// synchronized (failLock) {
	failed = true;
	errorIndicator.setErrorCode(TIMEOUT_ERROR);
	errorIndicator.setErrorString("Call to remote server timed-out after "
    ...
    /**
     * Overwrite to calculate specific timeout.
     * 
     * @return
     */
    ...
    /**
     * Overwrite to create an active responder with the necessary specific
     * responder embedded within it.
     */
    protected abstract AsynchronousResponseHandler createActiveResponder() throws Exception;
    ...
    /**
     * Used by an {@link AsynchronousResponseHandler} to notify that the
     * response has arrived.
     */
    public void asynchronousResponseReceived(AsynchronousResponseHandler ar) {
	System.err.println("Recieved response from "+
			   (ar == activeResponder ? "ACTIVE" : "NON_ACTIVE")+" Responder: "+ar);
	if (ar != activeResponder) {
	    logger.create().extractCallInfo().info().level(2).msg(
								  "Ignoring unsolicited completion report from non-active AsynchHandler: " + ar).send();
			
	    return;
	}

	// OK, its the current chappie...
	done = true;
	freeLock();

    }

    public void asynchronousErrorReceived(AsynchronousResponseHandler ar, int errorCode, String errorMessage) {
	if (ar != activeResponder) {
	    logger.create().extractCallInfo().info().level(2).msg(
								  "Ignoring unsolicited failure report from non-active AsynchHandler: " + ar).send();
	    return;
	}
     ...
     */
    public void abort() {
	logger.create().extractCallInfo().info().level(2).msg("Got Abort signal").send();
	aborted = true;
    ...
    /**
     * Override to perform special completion work.
     */
