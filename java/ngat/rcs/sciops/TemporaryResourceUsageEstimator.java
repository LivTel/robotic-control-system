 ...
 * 
 */
public class TemporaryResourceUsageEstimator implements ExecutionResourceUsageEstimationModel {
    ...
    public static final double IO_O_DEFAULT_READOUT_TIME = 15000.0;
	 ...
	 * 
	 */
	public TemporaryResourceUsageEstimator() {
	}
	...
	public ExecutionResourceBundle getEstimatedResourceUsage(GroupItem group) {
		...
		int expCount = getTotalObsCount(group.getSequence());
		ExecutionResource expCountRes = new ExecutionResource("EXPCOUNT", expCount);
                erb.addResource(expCountRes);

		return erb;

	}

    private int getTotalObsCount(ISequenceComponent seq) {
	...
	if (seq instanceof XIteratorComponent) {
            ...
            while (il.hasNext()) {
                ...
                numobs += getTotalObsCount(sc);
            }
            return count * numobs;
        } else if (seq instanceof XBranchComponent) {

            numobs += getTotalBranchCount((XBranchComponent) seq);

        } else if (seq instanceof XExecutiveComponent) {

            numobs += getTotalExposureCount((XExecutiveComponent) seq);
    ...
    private int getTotalBranchCount(XBranchComponent bran) {
	...
	int branchTotal = 0;
        ...
        while (il.hasNext()) {
            ISequenceComponent sc = (ISequenceComponent) il.next();
            branchTotal += getTotalObsCount(sc);
        }
    ...
    /** Count exposures.*/
    private int getTotalExposureCount(XExecutiveComponent exec) {
	...
	if (exposure instanceof XMultipleExposure) {
	    ...
	    return (int)((duration-15000)/exp); // add some time on for setup
	}
	 ...
	 * @param seq The sequenc element to evaluate.
	...
	private double getShutterTotal(ISequenceComponent seq) {
		...
		if (seq instanceof XIteratorComponent) {
		    ...
		    total += getBranchShutterTime((XBranchComponent) seq);
    ...
    private double getBranchShutterTime(XBranchComponent bran) {
        ...
        while (il.hasNext()) {
            ISequenceComponent sc = (ISequenceComponent) il.next();
            branchTotal += getShutterTotal(sc);
        }
	...
	/** For now we estimate the longest branch with no consideration of compatibility.*/
	...
	private double calculateReadoutTime(IInstrumentConfig config) {
		...
		if (instName.equals("RATCAM")) {
			return 1000.0 * (CCD_READOUT_TIME_MULT_FACTOR / binxy + CCD_READOUT_TIME_FIXED_OVERHEAD);
